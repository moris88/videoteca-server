...
function send_newsletter($data, $elencoFilm){
    //print_r($data); //TEST
    $name = 'Videoteca Admin';
    ...
    try {
        ...
        $mail->AltBody = 'Vai al sito http://192.168.1.208 per gli ultimi film inseriti come novit&#225;';
...
function send_email($data, $psw_temp){
    //print_r($data); //TEST
    $name = 'Videoteca Admin';
    $nickname = $data->nickname;
    $toEmail = $data->email;
    ...
    try {
        ...
        <body> 
            <p>Messaggio inviato da: '.$name.' a '.$nickname.'</p>
        ';
        $mail->Body .= '<h1> la tua password temporanea &#232;: '.$psw_temp.'</h1>';
        $mail->Body .= '<a href="http://192.168.1.208/#/home/login">Vai al login del sito web</a><br>';
        $mail->Body .= '---NON RISPONDERE A QUESTA EMAIL---<br>';
        $mail->Body .= '</body></html>';

        $mail->AltBody = 'la tua password temporanea &#232; '.$psw_temp;
